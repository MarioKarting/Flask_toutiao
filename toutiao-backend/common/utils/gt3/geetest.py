...
class GeetestLib(object):
    ...
    def _make_response_format(self, success=1, challenge=None, new_captcha=1):
        if not challenge:
            challenge = self._make_fail_challenge()
        if new_captcha:
            _format = {'success': success, 'gt': self.captcha_id, 'challenge': challenge, "new_captcha": True}
        else:
            _format = {'success': success, 'gt': self.captcha_id, 'challenge': challenge, "new_captcha": False}
        return _format
